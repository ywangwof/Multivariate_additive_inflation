...
!!  read ensembles
    ...
    if(mype==io_pe) allocate(en_full(iasm:iaemz,jasm:jaemz,kasm:kaemz,masm:maemz))
    if(mype==0)allocate(en_full_all(iasm:iaemz,jasm:jaemz,kasm:kaemz,mbsm:mbemz))
...
22  format('wrf_en',i3.3)
    ...
    allocate(rcounts(npe), displs(npe),scounts(npe))
    rcounts=0
    rcounts(0)=nlat*nlon*n2d
    rcounts(16)=nlat*nlon*n2d
    rcounts(32)=nlat*nlon*n2d
    rcounts(48)=nlat*nlon*n2d
    scounts=rcounts
    displs=0
    displs(0)=0
    displs(1:16)=nlat*nlon*n2d+1
    displs(17:32)=nlat*nlon*n2d*2+1
    displs(33:48)=nlat*nlon*n2d*3+1
    displs(49:63)=nlat*nlon*n2d*4+1
    if ( mas == mae ) then
    ...
    call mpi_gatherv(en_full(:,:,:,maemz), scounts(mype), mpi_rtype, &
                    en_full_all, rcounts(mype), displs(mype), mpi_rtype, 0, mpi_comm_world,ierror)
    !if(mype==16) call mpi_gather(en_full(:,:,:,maemz), nlat*nlon*n2d, mpi_rtype, &
    !                en_full_all, nlat*nlon*n2d, mpi_rtype, 0, mpi_comm_world,ierror)
...
! scatter to subdomains:
        ...
        do n = 1, n_ens
          ...
          print*,n,k
            if(mype==0) call fill_regional_2d(en_full_all(:,:,k,n),wrk_fill_2d)
            call mpi_scatterv(wrk_fill_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype, &
                              en_loc3(:,:,k,n),grd_ens%ijn_s(mype+1),mpi_rtype,0,mpi_comm_world,ierror)
          end do
        end do
...
!    call genex(s_a2b,en_full,en_loc)
    ...
    if(mype==io_pe)deallocate(en_full)
    if(mype==0)deallocate(en_full_all)
...
! u,v,t,q,w,dbz,qr,qs,qi,qg,qc,qnc,qni,qnr
    ...
    enddo
      ...
      write(6,*) 'Checking U....mype=',mype,maxval(u),minval(u)
      write(6,*) 'Checking V....mype=',mype,maxval(v),minval(v)
...
subroutine parallel_read_wrfarw_state_(en_full,m_cvars2d,m_cvars3d,nlon,nlat,nsig, &
   ...
   !open(99, file=trim(filename),form='binary')
     read(99)work
