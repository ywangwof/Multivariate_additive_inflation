...
!$$$
  ...
  use m_berror_stats_reg, only: berror_get_dims_reg,berror_read_wgt_reg_extra
...
! Declare local variables
  integer(i_kind) k,i,ii,ibin
  integer(i_kind) n,nn,nsig180
  integer(i_kind) j,k1,loc,kb,mm1,ix,jl,il
  integer(i_kind) inerr,l,lp,l2,bin_l
  integer(i_kind) msig,mlat,num_bins2d              ! stats dimensions
  integer(i_kind),dimension(nnnn1o):: ks
  integer(i_kind) nrf3_dbz,nrf3_oz,nrf2_sst,nrf3_cw,istatus
...
! Read dimension of stats file
  inerr=22
  call berror_get_dims_reg(msig,mlat,num_bins2d,inerr)
...
! note:  parameter vs needs to be inverted
  ...
  if(istatus/=0 .and. if_staticB_3d ) then
  ...
  !       print*,'ibin,vz',ibin,vz(1:10,:,ibin,nrf3_dbz)
  !       print*,'corz',corz(:,1:10,ibin,nrf3_dbz)
  !     end do
  !   end if
  !end do
           ...
           do i=1,lat2
              ...
              do k=1,nsig
                 ...
                 if( bintype == 5 )then
                   ...
                   if( abs(ges_mask(j,i,k)) > 0.5_r_kind )then
                     dssv(j,i,k,n)=dl1*dsv(i,k,2,l)+dl2*dsv(i,k,2,l2)
                   else
                     dssv(j,i,k,n)=0.0_r_kind ! dl1*dsv(i,k,1,l)+dl2*dsv(i,k,1,l2)
                   end if
                 else if ( bintype == 7 )then
                    if( ges_mask(i,j,k) < -0.5_r_kind )then
                      ...
                      dssv(i,j,k,n)=0.0_r_kind !(dl1*ozmzt(l,k)+dl2*ozmzt(l2,k))*dsv(1,k,1,llmin)
                    end if
                 end if
              end do
           end do
