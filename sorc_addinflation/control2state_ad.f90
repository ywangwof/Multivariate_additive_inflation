...
use obsmod,only: vordiv_cv, q_cv
...
character(len=3), parameter :: mycvars(ncvars) = (/  &
                               'sf ', 'vp ', 'ps ', 't  ', 'q  ',&
                               'cw ', 'ql ', 'qi ', 'u  ', 'v  ' /)
logical :: lc_sf,lc_vp,lc_ps,lc_t,lc_rh,lc_cw,lc_ql,lc_qi,lc_u,lc_v
...
logical :: do_getuv,do_tv_to_tsen_ad,do_normal_rh_to_q_ad,do_getprs_ad,do_cw_to_hydro_ad,do_uv_copy
...
do_normal_rh_to_q_ad=lc_t .and.lc_rh.and.ls_prse.and.ls_q.and.(.not.q_cv)
do_getprs_ad        =lc_t .and.lc_ps.and.ls_prse
do_uv_copy          =lc_u .and.lc_v

! --- test 
!do_normal_rh_to_q_ad=.false.
...
!$omp section
   ...
   if(do_uv_copy)then
   !  call gsi_bundlegetpointer (wbundle,'u' ,rv_u ,istatus)
   !  call gsi_bundlegetpointer (wbundle,'v' ,rv_v ,istatus)
   else
...
!  background error
   if (do_getuv) then
       if (twodvar_regional .and. icsfwter>0 .and. icvpwter>0) then
           ...
           if(.not.vordiv_cv)then
             call getuv(rv_u,rv_v,cv_sf,cv_vp,1)
             !print*,'ACV',maxval(cv_sf),maxval(cv_vp)
           else
             !print*,'BCV',maxval(cv_sf),maxval(cv_vp)
             call getuv_vordiv(rv_u,rv_v,cv_sf,cv_vp,1)
             !print*,'ACV',maxval(cv_sf),maxval(cv_vp)
             !cv_sf =  cv_sf + rv_sf * 1.0e-3
             !cv_vp =  cv_vp + rv_vp * 1.0e-3
             !call gsi_bundleputvar ( wbundle, 'sf',  cv_sf,   istatus )
             !call gsi_bundleputvar ( wbundle, 'vp',  cv_vp,   istatus )
           end if
       endif
   endif
...
!     e.g. cw-to-cw
    ...
    !        if( trim(clouds(ic)) == 'dbz' )then
    !        else
    !           rv_rank3 = rv_rank3 * 1.0e-5
    !        end if
         endif
      enddo
   end if
