...
contains  
  ...
  !$$$
    ...
    use obsmod, only   : if_model_dbz,if_staticB_3d, loghydro,lh,lv,iseed
    use perturb_dbz, only : add_smooth_perturbations
  ...
  ! rmse stuff
    ...
    real(r_single),allocatable::field3u(:,:,:),field3v(:,:,:),field1a(:),field3tmp(:,:,:),field3tmp1(:,:,:)
    real(r_single),allocatable::ht(:,:,:)
  ...
  !-------------  get grid info
       ...
       allocate(field3tmp(nlon_regional,nlat_regional,nsig_regional))
       allocate(field3tmp1(nlon_regional,nlat_regional,nsig_regional))
       allocate(ht(nlon_regional,nlat_regional,nsig_regional))
       ...
       do k=1, nsig_regional
         do j=1, nlat_regional
           do i=1, nlon_regional
             ht(i,j,k) = ( field3(i,j,k) + field3(i,j,k+1) ) / (2.0*9.81)
           enddo
         enddo
       enddo
       k=1
...
!      rmse_var='LANDMASK'
       ...
       if( if_staticB_3d ) then
          ...
          call ext_ncd_read_field(dh2,DateStr1,TRIM(rmse_var),              &
               field3tmp1,WRF_REAL,0,0,0,ordering,           &
               staggering, dimnames ,               &
               start_index,end_index,               & !dom
               start_index,end_index,               & !mem
               start_index,end_index,               & !pat
          ...
          do k=1,nsig_regional
           do i = 1, nlon_regional
            do j = 1, nlat_regional
              if( abs(field3tmp1(i,j,k)) > 90.0 )then
                   field3tmp1(i,j,k) = 1.0
              else
                   field3tmp1(i,j,k) = 0.0
              end if
      ...
      if( dbz_exist .and. if_model_dbz ) then
       ...
       !call add_smooth_perturbations(field3, field3tmp1, nlon_regional, nlat_regional, nsig_regional, &
       !                              iseed,lh,lv,one_single/rdx,one_single/rdy,ht,5.0)
       ...
       deallocate(field1,field1a,field2,field2b,field2c,ifield2,field3,field3u,&
                  field3v,field3tmp,field3tmp1)
  ...
  !$$$ end documentation block
    ...
    use control_vectors, only : w_exist, dbz_exist
    use obsmod, only   : if_model_dbz, loghydro
  ...
  ! Read in the rest of the fields
    ...
    if (l_cloud_analysis .or. n_actual_clouds>0) then
      ...
      write(6,*)' start_index=',start_index
      write(6,*)' end_index1=',end_index1
      where (field3 <= tiny_single) field3 = 0.0_r_kind
      call ext_ncd_write_field(dh1,DateStr1,TRIM(rmse_var),              &
           field3,WRF_REAL,0,0,0,ordering,           &
           staggering, dimnames ,               &
           start_index,end_index1,               & !dom
      ...
      write(6,*)' end_index1=',end_index1
      !where (field3 < tiny_single) field3 = tiny_single
      where (field3 <= 1.0_r_kind ) field3 = 0.0_r_kind
      call ext_ncd_write_field(dh1,DateStr1,TRIM(rmse_var),              &
           field3,WRF_REAL,0,0,0,ordering,           &
           staggering, dimnames ,               &
           start_index,end_index1,               & !dom
