...
!$$$ end documentation block
  ...
  use hybrid_ensemble_parameters, only: sqrt_beta_s, adaptive_hybrid
  ...
  use obsmod,only: if_staticB_3d
  use gsi_metguess_mod, only: gsi_metguess_bundle
  use obsmod,only: bintype
  ...
  do j=1,lon2
     do ii=1,nsubwin
        do ic3=1,nc3d
           ...
           do k=1,nsig
              do i=1,lat2
                if(if_staticB_3d .and. bintype /= 5 .and. adaptive_hybrid )then
                  if( abs(ges_mask(i,j,k)) < 3.5_r_kind ) then
...
!$$$ end documentation block
  use kinds, only: r_kind,i_kind
  use gsi_4dvar, only: nsubwin
  use hybrid_ensemble_parameters, only: n_ens,sqrt_beta_e,grd_ens,adaptive_hybrid
  use control_vectors,only: control_vector
  use timermod, only: timer_ini,timer_fnl
...
! Declare local variables
  ...
  ! Single resolution hybrid is allowed!
  call gsi_bundlegetpointer (gsi_metguess_bundle(1),'mask',ges_mask,istatus)
  ! Initialize timer
