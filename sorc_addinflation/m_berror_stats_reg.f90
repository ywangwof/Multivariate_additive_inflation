...
! !INTERFACE:
    ...
    module m_berror_stats_reg
      ...
      public :: berror_read_wgt_reg_extra ! get auto-cov.stats., prewgt()
...
! !INTERFACE:
    ...
    subroutine berror_get_dims_reg(msig,mlat,num_bins2d,unit)
...
! !INTERFACE:
    ...
    subroutine berror_read_bal_reg_extra(msig,mlat,num_bins2d,agvi,bvi,wgvi,evi2,evi3,mype,unit)
      ...
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: wgvi
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: bvi
      real(r_kind),dimension(20,0:mlat+1,num_bins2d,nsig)     ,intent(  out) :: evi2
      real(r_kind),dimension(100,0:mlat+1,num_bins2d,nsig,nsig),intent(  out) :: evi3
      integer(i_kind)                              ,intent(in   ) :: mype  ! "my" processor ID
      integer(i_kind),optional                     ,intent(in   ) :: unit ! an alternative unit
...
!         corp(:,n)=cov_dum(:,1)
        ...
        !end do
!        do i=0,mlat+1
!           hwllp(i,n)=hwll(i,1,nrf3_sf) !tentatively !#ww3
!           hwllp(i,n)=150000_r_kind  !
!        end do
     end if
     if (n==nrf2_tcamt) then
        do i=1,mlat
