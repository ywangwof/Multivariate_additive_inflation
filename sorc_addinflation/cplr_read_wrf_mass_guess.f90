...
!                                    read qnc
  ...
  !$$$
    ...
    use obsmod,only: if_model_dbz, if_staticB_3d
  ...
  ! Declare local variables
    integer(i_kind) kt,kq,ku,kv, kw,kw0,kdbz,kmask
  ...
  ! other internal variables
    ...
    integer(i_kind) i_qc,i_qi,i_qr,i_qs,i_qg,i_qnr,i_qni,i_qnc,i_w, i_dbz,i_mask
    integer(i_kind) kqc,kqi,kqr,kqs,kqg,kqnr,kqni,kqnc,i_xlon,i_xlat,i_tt,ktt
    integer(i_kind) i_th2,i_q2,i_soilt1,ksmois,ktslb
    integer(i_kind) ier, istatus
    integer(i_kind) n_actual_clouds
    ...
    real(r_kind), pointer :: ges_mask(:,:,:)=>NULL()
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
       ...
       if( if_staticB_3d ) num_mass_fields = num_mass_fields + lm
  ...
  ! hydrometeors
             ...
             if(if_staticB_3d) kmask=kmask+1
             if ( laeroana_gocart ) then
                if ( n_gocart_var > 0 ) then
                   do iv = 1, n_gocart_var
                      kchem(iv) = kchem(iv)+1
