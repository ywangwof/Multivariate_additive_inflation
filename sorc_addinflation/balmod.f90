...
! set passed variables to public
  ...
  integer(i_kind) ke_vp, extra_var2num, extra_var3num
...
!$$$
    ...
    use m_berror_stats_reg, only: berror_set_reg,berror_get_dims_reg,berror_read_bal_reg, &
                                  berror_read_bal_reg_extra
    use constants, only: zero,half,one
    use gsi_bundlemod, only: gsi_bundlegetpointer
    use gsi_metguess_mod, only: gsi_metguess_bundle
    use obsmod,only: bintype,if_staticB_3d
    implicit none
...
!   Declare local variables
    integer(i_kind) k,i,mlat,ivar
    integer(i_kind) j,m,lm,l,l2
    integer(i_kind) ke,inerr
    integer(i_kind) msig,num_bins2d                   ! stats dimensions
...
!   Read dimension of stats file
    inerr=22
    call berror_get_dims_reg(msig,mlat,num_bins2d)
...
!   Allocate arrays in stats file
    ...
    allocate ( evi2(20,0:mlat+1,num_bins2d,1:nsig) )
    allocate ( evi3(100,0:mlat+1,num_bins2d,1:nsig,1:nsig) )
...
!   below sigma level 0.8
    ...
    if (.not.twodvar_regional) then
       ...
       if( if_staticB_3d )  call gsi_bundlegetpointer(gsi_metguess_bundle(1),'mask',ges_mask,istatus)
       ...
       do k=1,nsig
          do j=1,lon2
             do i=1,lat2
                ...
                dl2=rllat1(i,j)-float(l)
                dl1=one-dl2
                if( ges_mask(i,j,k) > 3.5_r_kind ) ges_mask(i,j,k) = ges_mask(i,j,k) - 5.0
                if( bintype == 5 )then
                   evik2_real(ivar,i,j,k)=dl1*evi2(ivar,l,1,k)+dl2*evi2(ivar,l2,1,k)
                else if ( bintype == 6 )then
                   if( abs(ges_mask(i,j,k)) > 0.5_r_kind )then
                     evik2_real(ivar,i,j,k)=dl1*evi2(ivar,l,2,k)+dl2*evi2(ivar,l2,2,k)
                   else
                     evik2_real(ivar,i,j,k)= 0.0_r_kind
                   end if
                else
                   if( ges_mask(i,j,k) < -0.5_r_kind   )then
                     ...
                     evik2_real(ivar,i,j,k)=0.0_r_kind !dl1*evi2(ivar,l,1,k)+dl2*evi2(ivar,l2,1,k)
                   end if
                end if
             end do
          end do
                ...
                do i=1,lat2
                   ...
                   if( bintype == 5 )then
                     ...
                     if( abs(ges_mask(i,j,k)) > 0.5_r_kind  )then
                       ...
                       evik3_real(ivar,i,j,m,k)=0.0_r_kind
                     end if
                   else
                     if( ges_mask(i,j,k) < -0.5_r_kind  )then
                       evik3_real(ivar,i,j,m,k)=dl1*evi3(ivar,l,2,m,k)+dl2*evi3(ivar,l2,2,m,k)
                       ...
                       evik3_real(ivar,i,j,m,k)=0.0_r_kind !dl1*evi3(ivar,l,1,m,k)+dl2*evi3(ivar,l2,1,m,k)
                     end if
                   end if
                end do
             end do
...
!   for univariate surface analysis
    ...
    deallocate (agvi,bvi,wgvi,evi3)
    
    return
  end subroutine prebal_reg

  subroutine fulllist_var( varname, flag, varinlist )
    implicit none
    character(len=1024)       :: varname
    ...
    if( flag > 2 ) then
      ...
      select case(trim(varname))
        case("regcoeff_vor_div")
          varinlist=1
        case("regcoeff_vor_qs")
          varinlist=2
        case("regcoeff_vor_t")
          varinlist=3
        case("regcoeff_vor_w")
          varinlist=4
        case("regcoeff_vor_qcloud")
          varinlist=5
        case("regcoeff_vor_qice")
          varinlist=6
        case("regcoeff_vor_qrain")
          varinlist=7
        case("regcoeff_vor_qsnow")
          varinlist=8
        case("regcoeff_vor_qgraup")
          varinlist=9
        case("regcoeff_vor_dbz")
          varinlist=10
        case("regcoeff_div_u_qs")
          varinlist=11
        case("regcoeff_div_u_t")
          varinlist=12
        case("regcoeff_div_u_w")
          varinlist=13
        case("regcoeff_div_u_qcloud")
          varinlist=14
        case("regcoeff_div_u_qice")
          varinlist=15
        case("regcoeff_div_u_qrain")
          varinlist=16
        case("regcoeff_div_u_qsnow")
          varinlist=17
        case("regcoeff_div_u_qgraup")
          varinlist=18
        case("regcoeff_div_u_dbz")
          varinlist=19
        case("regcoeff_qs_u_t")
          varinlist=20
        case("regcoeff_qs_u_w")
          varinlist=21
        case("regcoeff_qs_u_qcloud")
          varinlist=22
        case("regcoeff_qs_u_qice")
          varinlist=23
        case("regcoeff_qs_u_qrain")
          varinlist=24
        case("regcoeff_qs_u_qsnow")
          varinlist=25
        case("regcoeff_qs_u_qgraup")
          varinlist=26
        case("regcoeff_qs_u_dbz")
          varinlist=27
        case("regcoeff_t_u_w")
          varinlist=28
      ...
      select case(trim(varname))
        case("regcoeff_vor_ps")
          varinlist = 1
        case("regcoeff_div_u_ps")
          varinlist = 2
        case("regcoeff_qs_u_ps")
          varinlist = 3
        case("regcoeff_ps_u_t")
          varinlist = 4
    ...
    ps_dbz=11
       ...
       do k=1,nsig
          do m=1,nsig
            do j=1,lon2
              do i=1,lat2
                dbz(i,j,k) = dbz(i,j,k) + evik3(qg_dbz,i,j,k,m)*qg(i,j,m)/qconst + &
                             evik3(qsnow_dbz,i,j,k,m)*qs(i,j,m)/qconst + &
                             evik3(qr_dbz,i,j,k,m)*qr(i,j,m)/qconst + &
                             evik3(qi_dbz,i,j,k,m)*qi(i,j,m)/qconst + &
                             evik3(ql_dbz,i,j,k,m)*ql(i,j,m)/qconst + &
                             evik3(w_dbz,i,j,k,m)*w(i,j,m)   + &
                             evik3(t_dbz,i,j,k,m)*t(i,j,m)   + &
                             evik2(ps_dbz,i,j,k)*ps(i,j)   + &
                             evik3(qs_dbz,i,j,k,m)*q(i,j,m)/qconst + &
       ...
       do k=1,nsig
          do m=1,nsig
            do j=1,lon2
              do i=1,lat2
                qg(i,j,k)  = qg(i,j,k) + &
                             evik3(qsnow_qg,i,j,k,m)*qs(i,j,m)/qconst + &
                             evik3(qr_qg,i,j,k,m)*qr(i,j,m)/qconst + &
                             evik3(qi_qg,i,j,k,m)*qi(i,j,m)/qconst + &
                             evik3(ql_qg,i,j,k,m)*ql(i,j,m)/qconst + &
                             evik3(w_qg,i,j,k,m)*w(i,j,m)   + &
                             evik3(t_qg,i,j,k,m)*t(i,j,m)   + &
                             evik2(ps_qg,i,j,k)*ps(i,j)   + &
                             evik3(qs_qg,i,j,k,m)*q(i,j,m)/qconst + &
            ...
            do j=1,lon2
              do i=1,lat2
                qs(i,j,k)  = qs(i,j,k) + &
                             ...
                             evik3(qs_qsnow,i,j,k,m)*q(i,j,m)/qconst+&
       ...
       do k=1,nsig
          do m=1,nsig
            do j=1,lon2
              do i=1,lat2
                qr(i,j,k)  = qr(i,j,k) + &
                             evik3(qi_qr,i,j,k,m)*qi(i,j,m)/qconst + &
                             evik3(ql_qr,i,j,k,m)*ql(i,j,m)/qconst + &
                             evik3(w_qr,i,j,k,m)*w(i,j,m)   + &
                             evik3(t_qr,i,j,k,m)*t(i,j,m)   + &
                             evik2(ps_qr,i,j,k)*ps(i,j) + &
                             evik3(qs_qr,i,j,k,m)*q(i,j,m)/qconst + &
            ...
            do j=1,lon2
              do i=1,lat2
                qi(i,j,k)  = qi(i,j,k) + &
                             ...
                             evik3(qs_qi,i,j,k,m)*q(i,j,m)/qconst
              end do
            end do
          end do
       end do
            ...
            do j=1,lon2
              do i=1,lat2
                ql(i,j,k)  = ql(i,j,k) + &
                             ...
                             evik3(qs_ql,i,j,k,m)*q(i,j,m)/qconst + &
            ...
            do j=1,lon2
              do i=1,lat2
                w(i,j,k)   = w(i,j,k) + &
                             ...
                             evik3(qs_w,i,j,k,m)*q(i,j,m)/qconst + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                div(i,j,m)   = div(i,j,m) + &
                               evik3(div_qs,i,j,k,m)*q(i,j,k)/qconst + &
                               evik2(div_ps,i,j,m)*ps(i,j) + &
                               evik3(div_t,i,j,k,m)*t(i,j,k)  + &
                               evik3(div_w,i,j,k,m)*w(i,j,k)  + &
                               evik3(div_ql,i,j,k,m)*ql(i,j,k)/qconst+ &
                               evik3(div_qi,i,j,k,m)*qi(i,j,k)/qconst+ &
                               evik3(div_qr,i,j,k,m)*qr(i,j,k)/qconst+ &
                               evik3(div_qsnow,i,j,k,m)*qs(i,j,k)/qconst+ &
                               evik3(div_qg,i,j,k,m)*qg(i,j,k)/qconst+ &
            ...
            do j=1,lon2
              do i=1,lat2
                t(i,j,m)     = t(i,j,m) + &
                               ...
                               evik3(t_ql,i,j,k,m)*ql(i,j,k)/qconst   + &
                               evik3(t_qi,i,j,k,m)*qi(i,j,k)/qconst   + &
                               evik3(t_qr,i,j,k,m)*qr(i,j,k)/qconst   + &
                               evik3(t_qsnow,i,j,k,m)*qs(i,j,k)/qconst   + &
                               evik3(t_qg,i,j,k,m)*qg(i,j,k)/qconst   + &
            ...
            do j=1,lon2
              do i=1,lat2
                ps(i,j)      = ps(i,j) + &
                               ...
                               evik2(ps_ql,i,j,k)*ql(i,j,k)/qconst   + &
                               evik2(ps_qi,i,j,k)*qi(i,j,k)/qconst   + &
                               evik2(ps_qr,i,j,k)*qr(i,j,k)/qconst   + &
                               evik2(ps_qsnow,i,j,k)*qs(i,j,k)/qconst   + &
                               evik2(ps_qg,i,j,k)*qg(i,j,k)/qconst   + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                q(i,j,m)     = q(i,j,m) + &
                               evik3(qs_w,i,j,k,m)*w(i,j,k)/qconst   + &
                               evik3(qs_ql,i,j,k,m)*ql(i,j,k)/qconst   + &
                               evik3(qs_qi,i,j,k,m)*qi(i,j,k)/qconst   + &
                               evik3(qs_qr,i,j,k,m)*qr(i,j,k)/qconst   + &
                               evik3(qs_qsnow,i,j,k,m)*qs(i,j,k)/qconst   + &
                               evik3(qs_qg,i,j,k,m)*qg(i,j,k)/qconst   + &
          ...
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                ...
                w(i,j,m)     = w(i,j,m) + &
                               evik3(w_ql,i,j,k,m)*ql(i,j,k)/qconst   + &
                               evik3(w_qi,i,j,k,m)*qi(i,j,k)/qconst   + &
                               evik3(w_qr,i,j,k,m)*qr(i,j,k)/qconst   + &
                               evik3(w_qsnow,i,j,k,m)*qs(i,j,k)/qconst   + &
                               evik3(w_qg,i,j,k,m)*qg(i,j,k)/qconst   + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                ql(i,j,m)    = ql(i,j,m) + &
                               evik3(ql_qi,i,j,k,m)*qi(i,j,k)/qconst + &
                               evik3(ql_qr,i,j,k,m)*qr(i,j,k)/qconst + &
                               evik3(ql_qsnow,i,j,k,m)*qs(i,j,k)/qconst + &
                               evik3(ql_qg,i,j,k,m)*qg(i,j,k)/qconst + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                qi(i,j,m)    = qi(i,j,m) + &
                               evik3(qi_qr,i,j,k,m)*qr(i,j,k)/qconst + &
                               evik3(qi_qsnow,i,j,k,m)*qs(i,j,k)/qconst + &
                               evik3(qi_qg,i,j,k,m)*qg(i,j,k)/qconst + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                qr(i,j,m)    = qr(i,j,m) + &
                               evik3(qr_qsnow,i,j,k,m)*qs(i,j,k)/qconst + &
                               evik3(qr_qg,i,j,k,m)*qg(i,j,k)/qconst + &
       ...
       do m=1,nsig
          do k=1,nsig
            do j=1,lon2
              do i=1,lat2
                qs(i,j,m)    = qs(i,j,m) + &
                               evik3(qsnow_qg,i,j,k,m)*qg(i,j,k)/qconst + &
