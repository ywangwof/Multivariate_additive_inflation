...
logical :: do_getprs_tl,do_normal_rh_to_q,do_tv_to_tsen,do_getuv,do_cw_to_hydro,do_uv_copy
...
lc_ql =icps(7)>0; lc_qi =icps(8)>0; lc_u  =icps(9)>0;lc_v  =icps(10)>0
...
do_normal_rh_to_q=lc_rh.and.lc_t .and.ls_prse.and.ls_q.and.(.not.q_cv)
do_tv_to_tsen    =lc_t .and.ls_q .and.ls_tsen
do_getuv         =lc_sf.and.lc_vp.and.ls_u.and.ls_v
do_uv_copy       =lc_u.and.lc_v

! --- test 
!do_normal_rh_to_q=.false.
...
!           print*,'CV',maxval(cv_sf),maxval(cv_vp)
   if(do_getuv) then
      if (twodvar_regional .and. icsfwter>0 .and. icvpwter>0) then
         ...
         else  ! inidicate cv_sf for vor, and cv_vp for div
!           cv_sf = cv_sf * 1.0e-3
!           cv_vp = cv_vp * 1.0e-3
           call getuv_vordiv(sv_u,sv_v,cv_sf,cv_vp,0)
         end if
      end if
   end if
...
!  Convert input normalized RH to q
   ...
   else ! specific humidity as control variable; for hybrid run, please set
        ! q_hyb_ens to true
     call gsi_bundlegetvar ( wbundle, 'q' , sv_q,  istatus )
     !sv_q = sv_q * 1.0e-3
   end if
