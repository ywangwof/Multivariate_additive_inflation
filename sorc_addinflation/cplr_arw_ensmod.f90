...
!$$$
    ...
    integer(i_kind):: i_radar_qr,i_radar_qg, i_sf, i_vp
  ...
  !    Determine if qr and qg are control variables for radar data assimilation,
    ...
    i_sf = 0
    i_vp = 0
    i_sf = getindex(cvars3d,'sf')
    i_vp = getindex(cvars3d,'vp')
...
! u,v,t,q,w,dbz,qr,qs,qi,qg,qc,qnc,qni,qnr
    do m=1,nc3d
       ...
       enddo
       if(trim(cvars3d(m))=='u' .or. trim(cvars3d(m))=='sf')  u    = scr3
       if(trim(cvars3d(m))=='v' .or. trim(cvars3d(m))=='vp')  v    = scr3
...
subroutine parallel_read_wrfarw_state_(en_full,m_cvars2d,m_cvars3d,nlon,nlat,nsig, &
  ...
  ! READ QR (kg/kg)
      ...
      if( loghydro )then
         where( temp_3d < smalllimit )
            temp_3d = smalllimit
         endwhere
         temp_3d = log10(temp_3d)
      end if
...
! u,v,t,q,w,dbz,qr,qs,qi,qg,qc,qnc,qni,qnr
   do k3=1,nc3d
     if(trim(cvars3d(k3))=='u' .or. trim(cvars3d(k3))=='sf') temp3(:,:,:,k3) = gg_u
     if(trim(cvars3d(k3))=='v' .or. trim(cvars3d(k3))=='vp') temp3(:,:,:,k3) = gg_v
